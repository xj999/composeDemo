...
class HybridAppRepositoryImpl(private val appDatabase: AppDatabase) : HybridAppRepository {
    override suspend fun fetchRemoteAppData(): ApiResponseEntity = withContext(Dispatchers.IO) {
        var hybrid: MutableList<HybridApp> = ArrayList()
        ...
        for (ti in res.tagAppList) {
            val tar = ti.hybridAppList.map { hybridAppList ->
                if (hybridAppList.appLogoUuid.isNotBlank()) {
                    ...
                    hybridAppList.copy(
                        appLogoUuid = "https://cdn.pixabay.com/photo/2023/11/21/21/38/puffins-8404284_1280.jpg",
        ...
        hybrid.addAll(myapp)
        saveTagList(tagList)
        saveHybridApp(hybrid)
        Timber.e("  当前线程 %s", Thread.currentThread().name)
    ...
    override suspend fun saveOrUpdateCommonApps(list: List<HybridApp>) =
